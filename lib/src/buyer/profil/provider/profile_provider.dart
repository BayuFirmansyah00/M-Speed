...
class ProfileProvider extends BaseController with ChangeNotifier {
  ...
  Future<void> fetchAkunSaya(BuildContext context,
    ...
    final response =
        await get(Constant.BASE_API_FULL + '/getakunsayabuyer', body: body);

    print('INI SELECTED DATE' + selectedDate.toString());
    print('INI SELECTED MONTH' + selectedMonth.toString());
    print('INI SELECTED YEAR' + selectedYear.toString());
  ...
  Future<void> fetchBuyer(BuildContext context,
      {bool withLoading = true, required String idBuyer}) async {
    if (withLoading) loading(true);

    final response = await get(Constant.BASE_API_FULL + '/getakunsayabuyer',
